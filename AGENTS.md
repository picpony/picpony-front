...
| Component motion | `spring-*` / `spring()` / `springTiming()`; anchored menus use `MENU_TRANSITION` and `menu-enter` / `menu-exit` | a duration and a curve chosen independently |
...
**Four primitives stand a default down when the call site names its own** — `Skeleton` for a radius, `Badge` for `max-w`, `ProgressBar` and `Field` for `w-full`. `Field` only stands down for an unconditional `w-*`; a breakpoint width such as `sm:w-32` keeps the full-width mobile default. The pattern matters because `cn` is a plain join: emitting both leaves Tailwind's output order to pick, which is not a decision anyone made.
...
**There are two text fields, and the label decides which.** They share the tone vocabulary, 56dp form height and `.m3-field` shell. The outlined field keeps its 4dp corner; the filled field takes an 8dp corner, matching `Select`'s default field shape. The search bar is the separate pill shape. The softer filled boundary is a deliberate project refinement, not a new M3 token claim.
...
The label and legend explicitly own their font weight and tracking as well as their size; the legend's tracking is also 0.75x, so surrounding emphasized text cannot change the notch. Label travel and notch opening share FastSpatial; their ink, the field boundary, button materials and selection-control colours use FastEffects. The general transition utility remains the 200ms standard fallback, and the state layer keeps its own 150ms response.
...
**Focus follows the field's boundary.** The outlined field has no outer ring: its own outline thickens to 2px in `primary-ink`, avoiding two nested boundaries. The filled field has no outline to nest inside, so it takes the ordinary `focus` ring. Both key off `:focus-within`, because the element wearing the indicator is the container and it is reporting on the control inside it. An invalid outlined field retains its error outline and label; on a filled field, the focus ring takes precedence over the error ring.
...
`Input` has **one height for a form slot and one for chrome**. Both the labelled and the unlabelled field are M3's 56dp, and `size="lg"` is M3's *search bar* — also 56dp, but a pill — for the single field on /search that is the whole point of its page. Boundary and corner follow the field's role; height stays shared. A labelled field with nothing to suggest is given a single-space placeholder, because `:placeholder-shown` is what tells the label whether the field is empty and it only matches while a placeholder exists.

`size="sm"` is the other axis — 40dp with `body-m`, for a field that is *chrome*: an admin filter, a control in an `.m3-row`. It shares the 40dp height, 8dp corner and 16dp text inset with `Select size="sm"`; the dropdown uses `label-l` because its value is a control label, while typed text stays `body-m`. Both are 14px. M3 offers no 40dp field, so this is a **stated divergence**, argued by the enclosure. It is unlabelled-only, because the notch's legend width, float distance and negative fieldset inset are all derived from 56dp, and a labelled field lives in a form column anyway. It takes **no `trailing` slot** — that inset is `(56 − 40) / 2`, so a 40dp box has no room to centre a 40dp control, and forbidding it is what keeps "one inset for every field size" true rather than forking it. `SearchInput` bakes it in rather than offering it, since every call site of that component is a filter bar.
...
**A field toolbar is one material.** `Button variant="surface"` uses `surface-container-highest` / `on-surface`, matching `Select` and the filled input. Use it for neutral commands beside fields, as in search's sort direction and advanced filters. These controls use the same 40dp step; a 32dp chip, a 40dp field and a pink tonal button are not one coherent toolbar. An expansion is a disclosure, not a selected filter: use `trailingIcon` with a rotating arrow and `aria-expanded` / `aria-controls`, rather than a `Chip` whose selection replaces the arrow with a tick.

`Select shape="pill"` is for a choice in that toolbar; the default `field` uses the filled field's 8dp corner. Search's bottom tools have no enclosing border or separator rules. The sort choice and direction form one group; advanced filters and reset form the other. These groups wrap together, keeping a reset from becoming a lone button on a new row. Expanded conditions sit on a quieter tonal surface, with spacing separating the two numeric conditions from the three attribute choices. The disclosure arrow shares the panel's opening `default-spatial` and closing `fast-effects` responses.

**Busy is active, disabled is unavailable.** A loading `Button` / `IconButton` keeps its material, ink and exact footprint while its spinner reports activity. Native `disabled` blocks duplicate activation; it does not imply the 0.38 visual treatment, which belongs to an actually unavailable control. Icons and text must not shift when busy begins, and an inherited spinner takes the control's own ink, including on media.
...
Its selected circle-to-square change uses `transition-icon-button`: FastSpatial for the corner and dismiss rotation, FastEffects for the material. The circular endpoint is 50%, rather than an oversized length whose radius stays visually clamped through most of the interpolation. This is selection feedback, not a press morph; the fixed square shape stays fixed. `Select`'s arrow instead uses `menu-enter` / `menu-exit`, the CSS twins of its popover's `MENU_TRANSITION`.
...
- **In `ColorPicker`, every cell is a real `Hct.from()` result, not a gradient**: sRGB's gamut in HCT is an irregular solid, so a gradient offers values the browser then clips and a user can aim at one colour and land on another. Chroma is a *fraction of what is available* at that hue and tone (at tone 95 a hue may hold a third of what it holds at 60). One `Pick` object holds the hex, hue and selected grid cell, updated atomically. The stored hue and cell are authoritative for selection: grey has no recoverable hue, and HCT rounding at light tones can map several cells to the same hex. Re-deriving the cell from that hex would move the check or select several cells; separate synchronising effects are unnecessary.
...
**Container steps follow the named M3 tones rather than a tint chosen per call site.** The elevation column records this app's shadow policy: ordinary cards keep their tonal variant without a decorative shadow, while detached overlays retain their shared elevation level.

| Component            | Container                     | Elevation |
| -------------------- | ----------------------------- | --------- |
| Filled card          | `surface-container-highest`    | 0         |
| `Card variant="elevated"` | `surface-container-low`    | 0         |
...
| Search suggestions   | `surface-container-highest`    | 1         |
...
The **outlined text field** has no outer ring: focus thickens its own outline to 2px in `primary-ink`, while an invalid outlined field retains the `error` outline and label. A control whose visual boundary *is* a 1px outline cannot wear a ring around it without reading as two nested boxes. The *filled* field has no outline to nest inside, so it takes the ordinary `focus` ring, which takes precedence over its error ring. This is the field family's own boundary treatment; it does not change the focus roles of other controls.
...
**A filter control that is not selected is a tone step, not a keyline.** `Select`, the filled text field and `Button variant="surface"` share `surface-container-highest` without a border or decorative shadow. An unselected `Chip` remains `surface-container-high`, a different control role. Use the field toolbar family when those neighbours need one material, rather than adding a chip just because the control expands something.
...
| Search suggestions (`Popover variant="search"`) | `rounded-2xl` | 28dp |
| Card, section surface                           | `rounded-md`   | 12dp  |
| Square icon button (the back affordance)        | `rounded-md`   | 12dp  |
| Chip, small tag                                 | `rounded-sm`   | 8dp   |
| Outlined text field, colour swatch, one-time-code box | `rounded-xs` | 4dp |
| Filled text field, `Select shape="field"`       | `rounded-sm`   | 8dp   |
| Menu / listbox row, ordinary autocomplete row   | `rounded-sm`   | 8dp   |
| Menu, popover, ordinary autocomplete panel      | `rounded-lg`   | 16dp  |
...
A **snackbar is 4dp** (`corner-extra-small`): a transient message, distinct from the rounder menu surface you act inside. The **navigation drawer rounds only its trailing corners**, and only while modal — a docked drawer is flush with the edge it docks to, so the rounding is `rounded-r-lg md:rounded-none`.
...
**Menu geometry is one nested family: 16dp panel, 8dp inset, 8dp row corner.** `Select`, `Menu` and ordinary autocomplete lists put 8dp padding on the panel; `Popover`'s content wrapper supplies the 2dp inter-row gap. Rows have a 40dp minimum under a fine pointer and 48dp under a coarse pointer. `estimateMenuHeight` includes both the insets and these gaps; search suggestions pass their own 56dp row estimate.

`Select` matches its trigger's width. Its row's 8dp inline padding plus the panel's 8dp inset align labels with the trigger's 16dp inset. The trailing mark reserves a 20dp slot, like the trigger arrow, and draws a 16dp `CheckGlyph weight="light"` inside it; selection changes opacity without adding width. A command menu may be intrinsically wider than its trigger: `matchAnchorWidth={false}` centres it on the trigger, then clamps it within the viewport. Do not widen a `Select` menu independently to hide an alignment problem.

**These corners are deliberate project refinements, not verbatim M3 component mappings.** The token sources still assign `CornerExtraSmall` to `MenuTokens.ContainerShape` and `OutlinedTextFieldTokens.ContainerShape`, and `CornerExtraSmallTop` to filled fields. The outlined field keeps 4dp; this app chooses 8dp filled fields and 16dp menus with inset rows for the rounder, coherent surface requested here. Search suggestions keep their separate 28dp / 20dp / 8dp geometry.

When citing what the specification says, **read the token file, not a summary table**, and distinguish it from these stated divergences. The generated `androidx.compose.material3.tokens` sources are fetchable —
...
| Object                                   | Level |
| ---------------------------------------- | ----- |
| Modal bottom sheet, search suggestions   | `e1`  |
| Menu popover / ordinary autocomplete, floating nav | `e2` |
| Dialog, FAB, snackbar                    | `e3`  |

**Ordinary controls and cards do not cast shadows, including on hover.** The user's visual direction is grounded tonal surfaces: buttons respond through their state layer and material, and `Card` variants through the surface scale. `elevated` remains a lighter surface variant for compatibility, with no rest or hover shadow. A pagination number, slider handle and table row are flat too. Shadows remain appropriate for a detached overlay whose overlap needs explaining — dialogs, sheets, menus and the floating nav — and stay on the shared elevation ladder. Do not blanket-remove box shadows: the keyboard ring and crop mask use that CSS mechanism without being elevation effects.

**The search suggestion view belongs to the search bar.** `Popover variant="search"` owns its matching width, 28dp corner, `surface-container-highest` and restrained `e1` overlap cue. Its 56dp suggestion rows sit 8dp inside the panel with 20dp corners and 2dp gaps. They appear on FastEffects (108ms at default speed) without scaling the text or staging a second child entrance; typing is a frequent action. Ordinary menu popovers use the 16dp outer shape and anchored menu transition described below.
...
**The motion family follows the object.** Page transitions use an easing and a duration; handles, indicators and small state changes use the spring roles below. Anchored menus are an explicit project exception: their measured trigger-to-panel transform follows Vuetify's arrangement on `MENU_TRANSITION`, using the app's own duration and curve tokens. That exception does not change a switch handle or tab indicator into a menu transition.
...
The assignments, with component sources and project-specific transition choices distinguished:
...
| Menu, listbox, ordinary autocomplete | `MENU_TRANSITION`, 200ms enter / 150ms exit | Anchored menu transition below |
| Search suggestions | `fast-effects`, opacity only | `Popover variant="search"` |
...
- Web Animations takes an `easing:` string, where a CSS `var()` is not resolved. New callers use `EASE` / `MENU_TRANSITION` from `lib/motionTokens.ts` or `springTiming()` from `lib/springTiming.ts`; `Modal`, `Popover` and the hero's chrome fades use these shared values. The top loader's `easing` prop in `app/layout.tsx` remains a literal token value; keep it in sync. `springTiming()` also applies the reduced tier's shape substitution, which a component's own cached spring string would miss.
...
These are the shared transition roles, and each base duration is on M3's 50ms grid. Reuse a role rather than choosing a number at a call site. The anchored menu deliberately uses the existing 200ms short step to enter and the 150ms state step to leave, through `MENU_TRANSITION` and its CSS twins; the state layer retains its own 150ms response.

**An anchored menu grows from the trigger's actual geometry.** `Popover` derives its translated and scaled collapsed pose from the trigger and the laid-out panel, in either opening direction. The reference is Vuetify's `VMenu` / `VDialogTransition` arrangement, not its literal timing values: this app enters over **200ms** on `standardDecelerate` and exits over **150ms** on `standardAccelerate`. `MENU_TRANSITION` is the WAAPI authority; `menu-enter` / `menu-exit` pair the same duration and curve for `Select`'s arrow. All clocks use the user's speed scale.

The panel and its content have different jobs on the same arrival. A fresh opening starts the content transparent and fades it on FastEffects once the surface is nearly expanded, finishing with the panel; a reversal starts immediately from the visible content's current opacity. Both in-flight poses are committed before cancellation. Under reduced motion the menu keeps one fade on the menu clock and drops the transform and separate content entrance. `Popover variant="search"` always uses a single FastEffects fade (108ms at default speed), and the off tier settles either variant immediately.
...
Three things the length has to fit: `HERO_PROGRESS_SAMPLES` is 48 samples per leg (47 intervals, 5.32ms each at 250ms); WAAPI interpolates those at the actual display refresh rate. The gallery card's chrome uses the shared 108ms FastEffects response: hiding finishes inside the flight, and restoring its marks after landing no longer spends another 200ms on a small effect. The reveal staircase is expressed as fractions of travel, so it cannot outlive the leg. Interruption poses interpolate the same cached numerical geometry that emitted the WAAPI transform keyframes, rather than evaluating a subtly different analytic position between samples; the continuous model still supplies signed velocity until landing, when velocity is zero. `npm test` compares the emitted transforms against poses at 120/144Hz, including the first few milliseconds of a negative-velocity reversal. The scaled 90ms reverse floor binds when the remaining-travel ratio approaches zero (the nominal floor would be 87.5ms at default speed).
...
**The body's reveal blends into its progress window.** At its delayed start the parent content is already partly visible; `u * u * (3 - 2 * u)` gives the local interval zero slope at both ends instead of stepping from rest straight into the flight's peak speed. This is a blend of the shared travel, with no separate clock. A continued/reversed reveal retains its current travel instead of starting another blend.

**A pull writes only the nodes that move or fade.** Inherited CSS variables at the overlay root made every pointer frame invalidate the full detail tree. `HeroPullSurface` caches the content, surface, reveal blocks and floating button, writes their transform/opacity, and refreshes them only on child-list changes so late metadata keeps the held pose. It restores the original inline styles at release. The gallery retains its existing scale pivot. Native scrolling and the stable gutter remain intact throughout the gesture. `npm run perf:hero` records production traces and an idle cadence reference; rAF timestamps are main-thread pacing, not a claim of physical 120fps. `npm run test:browser -- --hero-visuals` captures the real tracks at fixed times on 1440, 1920 and mobile viewports.
...
| Moves among `/`, `/search`, `/messages` | Shared axis from the route's mapped position, 500ms | `routeMove` / `playSharedAxis` |
| Any remaining route             | Cross-fade over an inert clone, 400ms | `lib/routeCrossFade.ts`          |
...
**Tabs.** Render `TabPanes` / `TabPane`; do not wire this by hand. Panes are marked, never unmounted — `data-tab-pane="name"` plus `data-tab-pane-active` — and stack in one CSS grid cell so both can be on screen at once. Never gate a pane on `hidden` or `{cond && ...}`: the outgoing one has to survive the commit or there is nothing to fade out. Never put a `key` on `[data-tab-panel]`; that deletes the animation's own targets in the commit that starts it. Panes must be written in the same order as the tabs above them — direction is derived from DOM order. On the home page, call `startTabTransition` and the native history update in the same tick, as the URL rule above requires; screens whose tabs live in local state need nothing beyond `TabPanes`, because the state update and its layout effect are the same commit.
...
**A route owns the entrance of the whole incoming page.** `RouteCrossFade` snapshots the outgoing page in `getSnapshotBeforeUpdate`, before React mutates the DOM, and sets `data-route-transit` on the persistent app scroller before the incoming children's layout effects run. `Reveal` and the gallery's `useStaggerGridOn` consult that marker; the grid also latches a skipped entrance so a later update cannot replay it. This keeps the route's motion from stacking with a fade or transform per child. Portalled dialogs outside the page remain independent. `settleEntryAnimations` finishes the page's finite entrance keyframes, leaving perpetual spinners and skeleton shimmer alone.

The plain route fade disables `pageIn` and drives the incoming page's opacity itself. `PAGE_FADE_TIMING` is **80ms of overlap delay plus 320ms of fading**, totaling the same 400ms as the cold entry token. It adds no incoming translation that could corrupt a gallery card's hero measurement. Mapped route moves use the shared axis on the whole page with `lean: false`; both paths clear the transit marker on completion or cancellation.

**The route footer travels with its page; only tabs hold it aside.** It is already inside `[data-page-content]`, so hiding and re-entering it after a route adds a separate, late arrival. `beginPageTransit` and `data-page-transit` are the tab-only hold, because tab panes sit above the footer and their final height changes at settlement. Their release uses the 166ms DefaultEffects fade above. Do not substitute this marker for the route entrance marker: they have different owners and jobs.
...
**A returning hero exposes the gallery to input immediately.** The runtime's `direction` distinguishes both meanings of `reversing`; `AppLayout` removes the gallery's `inert` when it is `back`, before the route commits. Keeping `inert` until the detail route unmounted sent a new wheel/touch stream into the non-scrollable host, and the close then waited for that same stream to stop. `HeroScrollContinuity` distinguishes the visible input target from the peer that last scrolled: fresh input drops old residuals before the browser's default scroll, and proves the handoff can finish without waiting for the new stream to go quiet. `setInputTarget` reattaches a peer that an outgoing event removed during the reveal frame. The closing route and its sibling back button share one inert lease; its eventual focus restore preserves any new focus the user chose in the gallery. Quiet, expiry and abort are explicit outcomes: a still-owned, non-aborted transfer proceeds on budget expiry instead of comparing a timer against a fractional deadline, which browsers can fire just before.

**An interrupted hero keeps the rendered clock and pose.** Read WAAPI `currentTime` for interruption; fades and the header's in-flight rise start from their current values. A viewport rebuild reschedules the shared tracks on the same remaining clock unless a pull gesture already owns them. Reverse destinations remain the real host/card, even after a rebase replaced the leg's start with an intermediate pose. A continued background scale keeps its original pivot while below 1, or scrolling changes the pivot and produces a translation jump. The recognizer coalesces pull samples once; the surface writes that frame directly rather than queuing a second frame. `npm run test:hero` covers native wheel/touch at return boundaries, delayed-data Esc + scroll, reopens, focus, and interruption; `npm test` also exercises the pure continuity/velocity contracts.
...
**A pane split follows its content width.** Search filters and blocking groups use container queries; private messages need 42rem for an 18rem contact list and a 24rem conversation. Below that, contacts and the thread share one column, including on a tablet with the app drawer open. The contact-collapse preference survives resizing and only applies while both panes fit. The message composer is a deliberate 48dp dense textarea (28px line + 10px padding per side), with the same local 48dp floor on its accessories; ordinary form fields remain 56dp. Its height is recalculated on width changes as well as typing.
...
| The dense chat composer's field | 48 | `body-l` |
| The chat composer's emoji and send controls | 48, through the row's local touch floor | — |
| A filter bar, an admin surface, or inside an `.m3-row` | 40 | `body-m` for typed text; `label-l` for choices and commands |
| An app-bar action | 48 | — |
| Dense chrome: a table row's actions, a chip's dismiss cross | 32 | `label-l` |

The composer is the stated dense exception: its 28px prose line plus 10px block padding on each side makes a 48dp field. The row sets a local 48dp touch floor so both accessories match it at every pointer density. With 8dp around the trio, the one-line composer is 64dp tall; growing text increases the field until its scrolling cap. Ordinary form fields retain their 56dp baseline.

**The app bar sets its own 48dp floor**, so every `touch-size` inside its `<header>` lands at 48 whatever the pointer is. Those controls sit on a 64dp coloured band. The dense chat composer separately sets the same local floor to match its field; neither changes the global pointer-dependent floor for other controls.
...
**48 is not a general-purpose control size.** It is the touch floor and the pointer-side drawer row, plus the explicitly dense chat composer above. Do not turn that local exception into a second ordinary form-field height.
...
| 48 | a tab (`PrimaryNavigationTabTokens.ContainerHeight`), an app-bar action, the dense chat composer and its accessories, the drawer row under a pointer, and `--touch-floor` under a finger |
| 56 | `Button size="lg"`, a labelled or unlabelled `Input`, the search bar, `Select`, `ColorSwatch`, `CodeInput`, the drawer row under a finger |
...
One consequence: **the two ordinary text fields are the same height.** Their boundary and corner differ, as described in the Input section; both remain 56dp in a form. The dense steps belong to their own enclosures and do not change that form baseline.
...
Below 18 a standalone Material Symbol stops resolving clearly. **Supporting marks have explicit primitive-owned exceptions**: `Badge` uses 14 at `sm` and 16 at `md`; `Select` draws its 16dp light `CheckGlyph` inside a reserved 20dp slot. These marks accompany text and are not separate targets. The badge's line box is 16px, so an 18dp glyph would make a badge with an icon 2px taller than one without; the listbox's reserved slot similarly keeps selection from moving its label.
...
- **The cascade must not play on a cold load or compete with an active route entrance.** `/` renders its first feed page on the server, so on a cold load the cards are painted before the engine arrives and a late cascade would park visible cards at `autoAlpha: 0`. `StaggerGrid` decides **once, at mount**, whether the engine is already resident and never subscribes to its later arrival. Even with the engine resident, `useStaggerGridOn` checks the route's pre-commit marker before seeding any card transforms and latches the skip when that route owns the entrance. `warmMotion()` still fetches the engine on an idle callback for every other consumer.
...
**The home feed has one server cache: its 120-second process memo.** Its inner fetch is `no-store`. Next's persistent Data Cache can return an expired response while refreshing in the background; stamping that response with a new `generatedAt` made days-old pictures appear fresh to both the memo and the browser. Less-used developer-filter queries made the defect particularly visible. The memo still coalesces concurrent reads and serves prefetches, but an expired entry now waits for a real upstream response within the existing timeout.
