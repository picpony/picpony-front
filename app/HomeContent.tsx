...
function ImageList({ onRetry, seed }: { onRetry?: () => void; seed: FeedSeed | null }) {
  ...
  /* The first render uses exactly what the server used, so the hydration render matches the HTML
     — which is the whole reason these are `useState` seeded from the prop rather than direct reads
     of `localStorage`. After mount the device's own settings take over.

     When the two agree (the overwhelming case: the cookie was written by this browser) the seed
     applies, `read()` finds it fresh, and no request is sent. When they disagree — a cleared
     cookie, a setting changed in another tab, the first load after this shipped — the key changes
     after mount and one request goes out, which is exactly today's cost. `keepPrevious` keeps the
     server's rows on screen while it lands, so a disagreement is a refresh rather than an empty
     list, and `syncBrowsingCookie` heals it for next time. */
  /* `DEFAULT_BROWSING_FINGERPRINT`, not `''`. A missing seed (the server read timed out) left the
     first key on an fp no browser can ever compute — `browsingFingerprint()` always returns five
     `|`-joined fields — so the effect below always changed the key and `/` sent its content
     request twice, the first answer stored under a key nothing reads again. The default is what
     a device with no stored settings actually produces, so for the majority it matches. */
  ...
  useEffect(() => {
    ...
    /* Out of the effect body: `react-hooks/set-state-in-effect` rejects a synchronous setState
       here, which is the same reason `AppLayout`'s drawer-restore effect defers. */
    queueMicrotask(() => {
      syncBrowsingCookie();
      setFp(ownFp);
      setSort(ownSort);
  ...
  const read = useResource(
    homeFeed,
    { page, sort, fp },
    /* The seed is only ever offered for page 1 — it is what the server rendered. Page 2 has no
       seed and `initial.key` would not match anyway; passing `undefined` says so directly. */
  ...
  const handlePostClick = useCallback(
    (postId: number) => {
      /* No need to flush anything first: the tab bar writes its URL synchronously on the tap
         (`switchTab` in AppLayout), so `/?tab=forum` is already the current entry by the time a
         row can be pressed — which is what makes back from a thread return to the forum list. */
