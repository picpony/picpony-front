 ...
 * space 12 / 16 / 24 and icons at 20 / 20 / 24. The two display sizes above are
 ...
 *
 * **A button is flat at rest.** M3's filled button is elevation 0, level 1 on
 * hover only. Tonal/text/accent have no elevation in the spec.
 */
 ...
 * The glyph size belongs here, not at the call site: `[&>span>svg]` is author
 * CSS, which beats the svg's presentational `width` attribute, so an explicit
 * `size` on a bare icon is inert. The one exception is a glyph wrapped in an
 * element of its own (the app bar's theme-toggle `<span>`), where the attribute
 * is the only thing sizing it — do not remove it there. */
...
export function buttonClasses({
  ...
  return cn(
    ...
    /* Colour and shadow only, on `standard`/200ms — a recolour is not geometry,
       and there is no geometry here to animate (see the note above `SIZES`). */
